...
# NeMo RL virtual cluster placement-group retries (exponential backoff: 2^i seconds)
export NRL_VIRTUAL_CLUSTER_MAX_RETRIES=11
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/training_qwen35_27b_thrive_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    uv run python examples/run_vlm_sft.py \
        --config examples/configs/recipes/vlm/sft_qwen35_27b_thrive.yaml \
        2>&1 | tee -a "$LOG_FILE"
