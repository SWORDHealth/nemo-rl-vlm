...
#SBATCH --output=slurm_logs/nemo-rl-sft-qwen35-9b-thrive-%j.out
#SBATCH --error=slurm_logs/nemo-rl-sft-qwen35-9b-thrive-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_9b_thrive.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
chmod +x /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_9b_thrive.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_9b_thrive.sh
