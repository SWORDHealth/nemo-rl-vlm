#!/bin/bash
# SLURM batch script for NeMo RL Single-Node SFT Training
# 1 node with 8 GPUs

#SBATCH --job-name=nemo-rl-sft-qwen35-4b-thrive
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-sft-qwen35-4b-thrive-%j.out
#SBATCH --error=slurm_logs/nemo-rl-sft-qwen35-4b-thrive-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_4b_thrive.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
chmod +x /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_4b_thrive.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_4b_thrive.sh
