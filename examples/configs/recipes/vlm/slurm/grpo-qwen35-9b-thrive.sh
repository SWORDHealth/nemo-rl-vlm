#!/bin/bash
# SLURM batch script for NeMo RL Single-Node GRPO Training
# 1 node with 8 GPUs

#SBATCH --job-name=nemo-rl-grpo-qwen35-9b-thrive
#SBATCH --nodes=2
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-grpo-qwen35-9b-thrive-%j.out
#SBATCH --error=slurm_logs/nemo-rl-grpo-qwen35-9b-thrive-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_grpo_qwen35_9b_thrive.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    echo "=== Starting NeMo RL VLM GRPO Training ==="

    # Create a detailed log with timestamps and node info
    LOG_DIR="/home/pmartins/nemo-rl-vlm/slurm_logs/$(date +%Y%m%d)"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/grpo_qwen35_9b_thrive_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    uv run python examples/run_vlm_grpo.py \
        --config examples/configs/recipes/vlm/vlm_grpo_qwen35_9b_thrive.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_grpo_qwen35_9b_thrive.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_grpo_qwen35_9b_thrive.sh
