...
#SBATCH --job-name=nemo-rl-grpo-qwen35-4b-thrive
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-grpo-qwen35-4b-thrive-%j.out
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/grpo_qwen35_4b_thrive_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    uv run python examples/run_vlm_grpo.py \
        --config examples/configs/recipes/vlm/vlm_grpo_qwen35_4b_thrive.yaml \
        2>&1 | tee -a "$LOG_FILE"
