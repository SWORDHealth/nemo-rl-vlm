...
#SBATCH --job-name=nemo-rl-sft-qwen35-35b-a3b-thrive
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=128
#SBATCH --time=72:00:00
#SBATCH --output=slurm_logs/nemo-rl-sft-qwen35-35b-a3b-thrive-%j.out
#SBATCH --error=slurm_logs/nemo-rl-sft-qwen35-35b-a3b-thrive-%j.err

# Create a script that will be executed on each node via srun (in shared location)
cat > /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_35b_a3b_thrive.sh << 'WORKER_SCRIPT_EOF'
#!/bin/bash
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/training_qwen35_35b_a3b_thrive_node_${NODE_RANK}_$(date +%H%M%S).log"
    ...
    uv run python examples/run_vlm_sft.py \
        --config examples/configs/recipes/vlm/sft_qwen35_35b_a3b_thrive.yaml \
        2>&1 | tee -a "$LOG_FILE"
...
chmod +x /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_35b_a3b_thrive.sh
...
srun --ntasks-per-node=1 /home/pmartins/nemo-rl-vlm/examples/configs/recipes/vlm/slurm_multinode_worker_qwen35_35b_a3b_thrive.sh
